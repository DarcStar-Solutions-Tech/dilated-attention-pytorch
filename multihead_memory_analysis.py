...
class AttentionMemoryAnalyzer:
    ...
    def analyze_standalone_attention(
        ...
        if implementation == "original":
            ...
            intermediate_tensors = batch_size * seq_len * num_heads * head_dim * self.dtype_bytes * 3
        else:  # improved
            # More efficient tensor operations
            intermediate_tensors = batch_size * seq_len * num_heads * head_dim * self.dtype_bytes * 2
    ...
    def _calculate_attention_memory(
        ...
        for i, (seg_len, dil_rate) in enumerate(zip(config.segment_lengths, config.dilation_rates)):
            if seq_len < seg_len:
                continue
            ...
            qkv_size = 3 * batch_size * num_segs * effective_seq_len * heads_per_group * head_dim * self.dtype_bytes
    ...
    for i, config in enumerate(configs):
        ...
        for seq_len in sequence_lengths:
            ...
            for name, variant, impl_type in implementations:
                ...
                print(f"{name:<30} {profile.total:>6.1f}GB {profile.attention_computation:>6.1f}GB "
...
def estimate_max_tokens_all_variants():
    ...
    def estimate_transformer_memory(config, seq_len, attention_variant, optimization_level="baseline"):
        ...
        # Activation memory
        if optimization_level == "optimized":
            activation_gb = num_layers * embed_dim * seq_len * 2 * 0.1 / (1024**3)  # Gradient checkpointing
        else:
            activation_gb = num_layers * embed_dim * seq_len * 2 * 7 / (1024**3)  # Store all activations
        ...
        if attention_variant == "standalone_original":
            attn_profile = analyzer.analyze_standalone_attention(seq_len, attn_config, implementation="original")
        elif attention_variant == "standalone_improved":
            attn_profile = analyzer.analyze_standalone_attention(seq_len, attn_config, implementation="improved")
        elif attention_variant == "multihead_original":
            attn_profile = analyzer.analyze_multihead_attention(seq_len, attn_config, implementation="original")
        else:  # multihead_improved
            attn_profile = analyzer.analyze_multihead_attention(seq_len, attn_config, implementation="improved")
        ...
        total_gb = model_params_gb + optimizer_gb + activation_gb + attention_gb + gradient_gb + buffer_gb
    ...
    def find_max_tokens(config, attention_variant, optimization_level="baseline"):
        ...
        while min_tokens <= max_tokens:
            ...
            total_memory, _ = estimate_transformer_memory(config, mid_tokens, attention_variant, optimization_level)
    ...
    print("-" * 60)
    print(f"{'Model':<15} {'Standalone':<12} {'Standalone':<12} {'Multihead':<12} {'Multihead':<12}")
    ...
    for config in model_configs:
        ...
        print(f"{config['name']:<15} {row_results[0]:>8,} {row_results[1]:>8,} {row_results[2]:>8,} {row_results[3]:>8,}")
    ...
    for i, config in enumerate(model_configs):
        ...
        print(f"{config['name']}: Multihead overhead: {overhead:.1f}% "
    ...
    for i, config in enumerate(model_configs):
        ...
        print(f"{config['name']}:")
        print(f"  Standalone improvement: {standalone_gain:.1f}% ({standalone_orig:,} -> {standalone_imp:,})")
        print(f"  Multihead improvement: {multihead_gain:.1f}% ({multihead_orig:,} -> {multihead_imp:,})")
    ...
    for i, config in enumerate(model_configs):
        ...
        print(f"{config['name']}: {optimization_gain:.0f}x improvement "
