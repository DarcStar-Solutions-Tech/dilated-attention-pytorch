...
def benchmark_ring_attention_gpu(
    ...
    try:
        ...
        if rank == 0:
            ...
            print(f"  ✓ {implementation}: {time_ms:.1f}ms, Peak: {peak_memory:.3f}GB, Allocated: {allocated_memory:.3f}GB")
...
def run_single_gpu_baseline(seq_len: int, implementation: str) -> Dict:
    ...
    try:
        ...
        print(f"  ✓ {implementation} (1 GPU): {time_ms:.1f}ms, Peak: {peak_memory:.3f}GB, Allocated: {allocated_memory:.3f}GB")
...
def main():
    ...
    for seq_len in [4096, 8192, 16384, 32768]:
        ...
        # Find results
        single_results = [r for r in results if r['seq_len'] == seq_len and r['world_size'] == 1 and r['success']]
        multi_results = [r for r in results if r['seq_len'] == seq_len and r['world_size'] == 2 and r['success']]
        
        if single_results and multi_results:
            for impl in ["current", "fixed"]:
                single = next((r for r in single_results if r['implementation'] == impl), None)
                multi = next((r for r in multi_results if r['implementation'] == impl), None)
                
                if single and multi:
                    mem_reduction = (1 - multi['peak_memory_gb'] / single['peak_memory_gb']) * 100
                    speedup = single['time_ms'] / multi['time_ms']
                    
                    print(f"  {impl}:")
                    print(f"    1 GPU: {single['peak_memory_gb']:.3f}GB, {single['time_ms']:.1f}ms")
                    print(f"    2 GPUs: {multi['peak_memory_gb']:.3f}GB, {multi['time_ms']:.1f}ms")
    ...
    max_single = max([r['seq_len'] for r in results if r['world_size'] == 1 and r['success']], default=0)
    max_multi = max([r['seq_len'] for r in results if r['world_size'] == 2 and r['success']], default=0)
