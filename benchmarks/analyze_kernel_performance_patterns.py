...
def plot_performance_breakdown():
    ...
    for seq_len in results['seq_lens']:
        ...
        time_ms = benchmark_with_config(UnifiedHilbertAttentionOptimizedEnhanced, config, x)
        results['enhanced'].append(time_ms)
    
    # Create plot
    plt.figure(figsize=(10, 6))
    plt.plot(results['seq_lens'], results['unified'], 'o-', label='Unified', linewidth=2, markersize=8)
    plt.plot(results['seq_lens'], results['optimized'], 's-', label='Optimized', linewidth=2, markersize=8)
    plt.plot(results['seq_lens'], results['enhanced'], '^-', label='Enhanced', linewidth=2, markersize=8)
