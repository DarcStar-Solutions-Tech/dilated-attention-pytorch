#!/usr/bin/env python3
"""Benchmark multi-GPU scaling with patched ring attention."""
...
def benchmark_sequence_length(
    model, seq_len, batch_size, embed_dim, device, dtype, num_iters=5
):
...
def main():
    ...
    print(f"\n{'=' * 60}")
    print("Multi-GPU Ring Attention Scaling Benchmark")
    print(f"{'=' * 60}")
    ...
    # Save results
    if rank == 0:
        output_dir = Path("benchmarks/results/ring/multi_gpu_scaling")
        output_dir.mkdir(parents=True, exist_ok=True)

        output_file = output_dir / f"scaling_results_w{world_size}.json"
        ...
        print(
            f"{'Seq Len':<10} {'Throughput':<15} {'Memory (MB)':<15} {'KB/Token':<10}"
        )
        print("-" * 60)
        for r in results:
            print(
