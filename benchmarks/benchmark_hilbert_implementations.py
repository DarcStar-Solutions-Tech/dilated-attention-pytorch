...
class HilbertImplementationBenchmark:
    ...
    def benchmark_implementation(
        ...
        result = {
            ...
            "total_memory_mb": np.mean(total_memory) if total_memory else np.mean(forward_memory),
            "throughput_tokens_per_sec": (batch_size * seq_len) / (np.mean(forward_times) / 1000),
        }
        
        print(f"  Forward: {result['forward_time_ms']:.2f}±{result['forward_std_ms']:.2f} ms")
        print(f"  Backward: {result['backward_time_ms']:.2f}±{result['backward_std_ms']:.2f} ms")
        ...
        for seq_len in seq_lengths:
            ...
            for batch_size in batch_sizes:
                for num_heads in num_heads_list:
                    print(f"\n{'='*80}")
                    print(f"Configuration: seq_len={seq_len}, batch={batch_size}, heads={num_heads}")
    ...
    def generate_report(self, results: List[Dict[str, Any]], output_dir: str = "docs/benchmarks"):
        """Generate benchmark report."""
        timestamp = datetime.now(timezone.utc).strftime("%Y-%m-%d-%H%M-UTC")
        report_path = os.path.join(output_dir, f"hilbert-implementation-comparison-{timestamp}.md")
        ...
        with open(report_path, "w") as f:
            ...
            for seq_len in seq_lengths:
                ...
                f.write("| Implementation | Batch | Heads | Forward (ms) | Backward (ms) | Total (ms) | Memory (MB) | Throughput (tok/s) |\n")
                f.write("|----------------|-------|-------|--------------|---------------|------------|-------------|--------------------|\n")
                
                for r in seq_results:
                    f.write(f"| {r['name']} | {r['batch_size']} | {r['num_heads']} | ")
                    f.write(f"{r['forward_time_ms']:.2f}±{r['forward_std_ms']:.2f} | ")
                    f.write(f"{r['backward_time_ms']:.2f}±{r['backward_std_ms']:.2f} | ")
            ...
            f.write("1. **GPU-Optimized Implementation** provides the best performance with automatic backend selection\n")
            f.write("2. **Hilbert ordering** impact varies by sequence length and access patterns\n")
            f.write("3. **Pascal GPUs** (like GTX 1080) benefit from FP32 computation\n")
            f.write("4. **Modern GPUs** can leverage Flash Attention for significant speedups\n\n")
...
def main():
    parser = argparse.ArgumentParser(description="Compare Hilbert attention implementations")
