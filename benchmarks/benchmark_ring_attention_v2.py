            ...
            try:
                ...
                for _ in range(2):
                    if implementation == "StandardAttention":
                        scores = torch.matmul(q.transpose(1, 2), 
                                            k.transpose(1, 2).transpose(-2, -1)) / (head_dim ** 0.5)
                ...
                if device.type == "cuda":
                    ...
                    if implementation == "StandardAttention":
                        ...
                        theoretical = (3 * seq_len + seq_len * seq_len / num_heads) * \
                                    batch_size * num_heads * head_dim * 2 / (1024**2)
                    else:
                        # Q, K/V chunks, output
                        chunk_size = seq_len // ring_size
                        theoretical = (seq_len + 2 * chunk_size + seq_len) * \
                                    batch_size * num_heads * head_dim * 2 / (1024**2)
                ...
                print(f"{avg_time*1000:.1f}ms, {avg_memory:.1f}MB" 
                      if device.type == "cuda" else f"{avg_time*1000:.1f}ms")
    ...
    for impl in implementations:
        if impl == "StandardAttention":
            ...
            results = benchmark_implementation(
                impl, seq_lengths, [1], device=device
            )
        else:
            results = benchmark_implementation(
...
def create_comparison_plots(results: Dict):
    ...
    colors = {"StandardAttention": "blue", "RingAttentionCorrectV2": "green", 
    ...
    for impl_name, impl_results in results.items():
        ...
        if impl_name == "StandardAttention":
            ...
            if seq_lens:
                ax1.plot(seq_lens, memories, 'o-', label=impl_name, 
            ...
            for ring_size in [1, 4, 16]:
                ...
                memories = []
                for seq_len, ring_results in seq_results.items():
                    if ring_size in ring_results and "peak_memory_mb" in ring_results[ring_size]:
                        seq_lens.append(seq_len)
                        memories.append(ring_results[ring_size]["peak_memory_mb"])
                
                if seq_lens:
                    ax1.plot(seq_lens, memories, 'o--', 
        ...
        if impl_name != "StandardAttention":
            for seq_len in [4096, 16384]:
                if seq_len in seq_results and seq_len in std_results:
                    ...
                    if ring_sizes:
                        ax2.plot(ring_sizes, reductions, 'o-', 
        ...
        for ring_size in [1, 8]:
            ...
            for seq_len, ring_results in seq_results.items():
                if impl_name == "StandardAttention":
                    if 1 in ring_results and "throughput_tokens_per_sec" in ring_results[1]:
                        seq_lens.append(seq_len)
                        throughputs.append(ring_results[1]["throughput_tokens_per_sec"])
                elif ring_size in ring_results and "throughput_tokens_per_sec" in ring_results[ring_size]:
                    seq_lens.append(seq_len)
                    throughputs.append(ring_results[ring_size]["throughput_tokens_per_sec"])
            
            if seq_lens:
                label = impl_name if impl_name == "StandardAttention" else f"{impl_name} (ring={ring_size})"
                ax3.plot(seq_lens, throughputs, 'o-', label=label,
                        color=color, alpha=0.8)
        ...
        if impl_name != "StandardAttention":
            ...
            if seq_len in seq_results:
                ...
                if ring_sizes:
                    ax4.plot(ring_sizes, times, 'o-', label=impl_name,
                            color=color, linewidth=2, markersize=8)
...
def analyze_results(results: Dict):
    ...
    print(f"{'Implementation':<30} {'Time (ms)':<12} {'Memory (MB)':<12} {'Throughput':<15}")
    print("-" * 70)
...
def main():
    ...
    for impl_name, impl_results in results.items():
        ...
        for seq_len, ring_results in impl_results.get("seq_length_results", {}).items():
            summary["max_sequence_tested"] = max(summary["max_sequence_tested"], seq_len)
