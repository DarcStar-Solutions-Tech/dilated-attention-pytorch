...
def run_focused_benchmark():
    ...
    for seq_len, batch_size, num_heads, embed_dim in configs:
        ...
        try:
            ...
            result_hilbert = {
                ...
                "throughput_tps": (batch_size * seq_len) / (np.mean(forward_times) / 1000),
            }
            
            print(f"  Backend: {module_hilbert.attention_backend}")
            print(f"  Forward: {result_hilbert['forward_ms']:.2f}±{result_hilbert['forward_std']:.2f} ms")
            print(f"  Backward: {result_hilbert['backward_ms']:.2f}±{result_hilbert['backward_std']:.2f} ms")
        ...
        try:
            ...
            print(f"  Forward: {result_no_hilbert['forward_ms']:.2f}±{result_no_hilbert['forward_std']:.2f} ms")
            print(f"  Backward: {result_no_hilbert['backward_ms']:.2f}±{result_no_hilbert['backward_std']:.2f} ms")
    ...
    with open(report_path, "w") as f:
        ...
        f.write("| Seq Len | Hilbert | Backend | Forward (ms) | Backward (ms) | Total (ms) | Memory (MB) | Throughput (tok/s) |\n")
        f.write("|---------|---------|---------|--------------|---------------|------------|-------------|--------------------|\n")
        
        for r in results:
            f.write(f"| {r['seq_len']:,} | {'Yes' if r['use_hilbert'] else 'No'} | {r['backend']} | ")
        ...
        for seq_len in seq_lens:
            ...
            no_hilbert_result = next((r for r in seq_results if not r['use_hilbert']), None)
            ...
            if hilbert_result and no_hilbert_result:
                ...
                f.write(f"Hilbert is {speedup:.2f}x {'faster' if speedup > 1 else 'slower'}\n")
        
        f.write("\n## Conclusions\n\n")
        f.write("1. The GPU-optimized implementation automatically selects the best backend\n")
        f.write("2. Hilbert ordering impact varies by sequence length\n")
        f.write("3. Manual backend is used on Pascal GPUs (GTX 1080) for compatibility\n")
