...
def run_comparison_benchmark():
    ...
    for seq_len, ring_size in test_configs:
        ...
        try:
            ...
            memory_gb, time_ms, success = measure_memory_and_time(current_module, seq_len)
            if success:
        ...
        try:
            ...
            if success:
                ...
                print(f"    Theoretical memory per device: {mem_info['per_device_gb']:.3f}GB")
        ...
        if success:
            ...
            print(f"    Memory (per chunk): {chunk_memory_gb:.3f}GB, Total time: {total_time_ms:.1f}ms")
...
def main():
    ...
    if results["current"]["seq_lens"] and results["true"]["seq_lens"]:
        ...
        common_lens = set(results["current"]["seq_lens"]) & set(results["true"]["seq_lens"])
