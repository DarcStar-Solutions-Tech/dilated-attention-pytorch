...
# Import the two remaining implementations
...
def run_comprehensive_benchmark():
    """Run comprehensive benchmarks on the two implementations."""
    ...
    print(f"{'Config':<20} | {'Unified':<20} | {'Enhanced':<20}")
    print("-" * 65)
    ...
    for seq_len, dilation_rate, desc in test_configs:
        ...
        for impl_name in ["Unified", "UnifiedOptimizedEnhanced"]:
        ...
        for impl_name in ["UnifiedOptimized", "UnifiedOptimizedEnhanced"]:
            ...
            for desc, data in results[impl_name].items():
                if pattern in desc and data is not None:
                    baseline_key = desc
                    if (
                        baseline_key in results["Unified"]
                        and results["Unified"][baseline_key] is not None
                    ):
...
def plot_benchmark_results(results: Dict, test_configs: list):
    ...
    width = 0.25
    for i, impl_name in enumerate(
        ["Unified", "UnifiedOptimized", "UnifiedOptimizedEnhanced"]
    ):
        times = []
    ...
    for impl_name in ["UnifiedOptimized", "UnifiedOptimizedEnhanced"]:
        ...
        ax4.plot(
            x_pos, speedups, marker="o", label=impl_name, linewidth=2, markersize=8
        )
