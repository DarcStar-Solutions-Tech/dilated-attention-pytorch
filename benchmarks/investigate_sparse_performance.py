...
def profile_sparse_execution(
    impl_class, impl_name, seq_len, dilation_rate, hidden_dim=512, num_heads=8
):
    ...
    if hasattr(module, "dilation_rate") and module.dilation_rate > 1:
        if impl_name == "Enhanced":
            print("  Enhanced will use strided sparse attention path")
    ...
    total_ops = (
        batch_size * num_heads * seq_len * active_positions * (hidden_dim // num_heads)
    )
...
def analyze_kernel_paths():
    ...
    # Create input
    _ = torch.randn(2, seq_len, hidden_dim).cuda()
    ...
    if module.dilation_rate > 1:
        print("  Will use _strided_sparse_attention method")
...
def compare_sparse_implementations():
    ...
    print("\n\n=== Performance Summary ===")
    print(
        f"{'Config':<15} | {'Unified':<12} | {'Optimized':<12} | {'Enhanced':<12} | {'Best':<10}"
    )
    print("-" * 65)
...
def analyze_sparse_memory_patterns():
    ...
    print("  - Simple strided access pattern")
    print(f"  - Memory stride: {dilation_rate} elements")

    # Optimized approach (may have overhead)
    print("\n2. Optimized Approach:")
    print("  - May process in larger blocks")
    print("  - Additional complexity for block alignment")
    print("  - Potential overhead from optimization logic")

    # Enhanced approach (uses special sparse method)
    print("\n3. Enhanced Approach:")
    print("  - Uses _strided_sparse_attention method")
    print("  - PyTorch-based sparse computation")
    print("  - Additional overhead from Python loop")
