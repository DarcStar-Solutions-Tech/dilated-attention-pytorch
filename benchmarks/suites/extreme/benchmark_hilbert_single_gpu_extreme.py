    ...
    for seq_len in seq_lengths:
        ...
        for use_hilbert in [True, False]:
            try:
                ...
                print(
                    f"  Forward: {result['forward_ms']:.2f}±{result['forward_std']:.2f} ms"
                )
                print(
                    f"  Backward: {result['backward_ms']:.2f}±{result['backward_std']:.2f} ms"
                )
...
def generate_report(results: List[Dict]):
    ...
    with open(report_path, "w") as f:
        ...
        no_hilbert_success = [
            r["seq_len"] for r in successful if not r.get("use_hilbert")
        ]
        ...
        f.write(
            f"- **Overall Maximum**: {max(max_hilbert, max_no_hilbert):,} tokens\n\n"
        )
        ...
        if failed:
            for r in failed:
                f.write(
                    f"- {r['seq_len']:,} tokens ({'Hilbert' if r.get('use_hilbert') else 'No Hilbert'}): "
                )
                f.write(
                    f"{r.get('status')} - {r.get('error', r.get('reason', 'Unknown'))}\n"
                )
        ...
        for seq_len in seq_lengths:
            hilbert = next(
                (r for r in successful if r["seq_len"] == seq_len and r["use_hilbert"]),
                None,
            )
            no_hilbert = next(
                (
                    r
                    for r in successful
                    if r["seq_len"] == seq_len and not r["use_hilbert"]
                ),
                None,
            )
            ...
            if hilbert and no_hilbert:
                ...
                memory_ratio = (
                    hilbert["memory_mb"] / no_hilbert["memory_mb"]
                    if no_hilbert["memory_mb"] > 0
                    else 1.0
                )

                f.write(f"### {seq_len:,} tokens\n")
                f.write(
                    f"- **Speedup**: {speedup:.2f}x {'faster' if speedup > 1 else 'slower'} with Hilbert\n"
                )
                f.write(
                    f"- **Memory**: {memory_ratio:.2f}x memory usage with Hilbert\n"
                )
                f.write(
                    f"- **Throughput difference**: {(hilbert['throughput_tps'] - no_hilbert['throughput_tps']):+,.0f} tokens/sec\n\n"
                )
        ...
        f.write(
            "3. **Memory Efficiency**: Ring attention with single GPU still provides benefits\n"
        )
        f.write(
            "4. **Performance**: Throughput remains good even at extreme lengths\n\n"
        )

        f.write("## Recommendations\n\n")
        f.write(
            "1. **For > 32K tokens**: Consider multi-GPU setup for better performance\n"
        )
        f.write(
            "2. **Memory Limited**: Use gradient checkpointing for longer sequences\n"
        )
