    ...
    for seq_len in seq_lengths:
        ...
        try:
            ...
            if rank == 0:
                ...
                print(
                    f"  Effective tokens/sec/GPU: {result['throughput_tokens_per_sec'] / world_size:,.0f}"
                )
...
def generate_report(results: List[Dict[str, Any]], world_size: int):
    ...
    with open(report_path, "w") as f:
        ...
        f.write("- GPU Type: NVIDIA GeForce GTX 1080 (Pascal)\n")
        f.write("- Data Type: torch.float32\n")
        f.write("- Ring Communication: isend/irecv (proper pattern)\n\n")
        ...
        if seq_lengths:
            f.write(
                "| Sequence Length | Hilbert | Forward (ms) | Backward (ms) | Total (ms) | Memory/GPU (MB) | Throughput (tok/s) | Tokens/GPU |\n"
            )
            f.write(
                "|-----------------|---------|--------------|---------------|------------|-----------------|--------------------|-----------|\n"
            )
            ...
            for seq_len in seq_lengths:
                seq_results = [
                    r
                    for r in results
                    if r.get("seq_len") == seq_len and "error" not in r
                ]

                for r in seq_results:
                    f.write(
                        f"| {r['seq_len']:,} | {'Yes' if r['use_hilbert'] else 'No'} | "
                    )
        ...
        hilbert_results = [
            r for r in results if r.get("use_hilbert", False) and "error" not in r
        ]
        no_hilbert_results = [
            r for r in results if not r.get("use_hilbert", False) and "error" not in r
        ]

        max_hilbert = (
            max([r["seq_len"] for r in hilbert_results]) if hilbert_results else 0
        )
        max_no_hilbert = (
            max([r["seq_len"] for r in no_hilbert_results]) if no_hilbert_results else 0
        )
        ...
        if failed:
            for r in failed:
                f.write(
                    f"- {r['seq_len']:,} tokens ({'Hilbert' if r.get('use_hilbert') else 'No Hilbert'}): {r['error']}\n"
                )
        else:
        ...
        for seq_len in seq_lengths:
            ...
            if hilbert and no_hilbert:
                ...
                memory_ratio = (
                    hilbert["memory_mb_per_gpu"] / no_hilbert["memory_mb_per_gpu"]
                )

                f.write(f"### {seq_len:,} tokens\n")
                f.write(
                    f"- **Performance**: Hilbert is {speedup:.2f}x {'faster' if speedup > 1 else 'slower'}\n"
                )
                f.write(f"- **Memory**: Hilbert uses {memory_ratio:.2f}x memory\n")
                f.write(
                    f"- **Throughput gain**: {(hilbert['throughput_tokens_per_sec'] - no_hilbert['throughput_tokens_per_sec']):,.0f} tokens/sec\n\n"
                )

        f.write("## Key Findings\n\n")
        f.write(
            "1. **Ring Attention Scaling**: Successfully distributes computation across multiple GPUs\n"
        )
        f.write(
            "2. **Memory Efficiency**: Each GPU only needs to store its portion of the sequence\n"
        )
        f.write(
            "3. **Hilbert Benefits**: Most pronounced at extreme sequence lengths\n"
        )
        f.write(
            "4. **Communication Overhead**: Ring pattern adds latency but enables much longer sequences\n\n"
        )
        ...
        f.write(
            "4. **Consider communication**: Network bandwidth affects ring performance\n"
        )
...
def main():
    ...
    parser.add_argument(
        "--world-size", type=int, default=2, help="Number of GPUs to use (default: 2)"
    )
    parser.add_argument(
        "--seq-lengths",
        type=int,
    ...
    parser.add_argument(
        "--embed-dim", type=int, default=768, help="Embedding dimension (default: 768)"
    )
    ...
    if torch.cuda.device_count() < args.world_size:
        print(
            f"Error: Requested {args.world_size} GPUs but only {torch.cuda.device_count()} available"
        )
        return
