...
def main():
    ...
    print("=" * 100)
    print("HILBERT ATTENTION IMPLEMENTATION COMPARISON")
    print("=" * 100)
    print()
    ...
    for (
        ...
        # Calculate speedups
        if times["Original"] != float("inf"):
            print("\n  Speedup vs Original:")
            for name in ["Unified", "Unified Optimized", "Enhanced"]:
                if times[name] != float("inf"):
                    speedup = times["Original"] / times[name]
                    print(f"    {name:20s}: {speedup:6.2f}x")

        # Compare Enhanced vs others
        if times["Enhanced"] != float("inf"):
            print("\n  Enhanced performance:")
            for name in ["Unified", "Unified Optimized"]:
                if times[name] != float("inf"):
                    speedup = times[name] / times["Enhanced"]
                    if speedup > 1:
                        print(f"    vs {name:20s}: {speedup:6.2f}x faster")
                    else:
                        print(f"    vs {name:20s}: {1 / speedup:6.2f}x slower")
    ...
    print(
        f"{'Seq Len':>8} | {'Original':>10} | {'Unified':>10} | {'Unified Opt':>12} | {'Enhanced':>10} | {'Best':>15}"
    )
    print("-" * 80)
    ...
    for result in results:
        if result["dilation_rate"] == 1:
            ...
            print(f"{times.get('Enhanced', float('inf')):9.2f}ms | ", end="")
            print(f"{best_name:>15}")

    # Sparse attention summary
    print("\nSparse Attention (dilation_rate>1):")
    print(
        f"{'Seq Len':>8} | {'Dilation':>8} | {'Original':>10} | {'Enhanced':>10} | {'Speedup':>10}"
    )
    print("-" * 65)
    ...
    for result in results:
        if result["dilation_rate"] > 1:
            times = result["times"]
            if times["Original"] != float("inf") and times["Enhanced"] != float("inf"):
                speedup = times["Original"] / times["Enhanced"]
                print(
                    f"{result['seq_len']:8d} | {result['dilation_rate']:8d} | ", end=""
                )
                print(f"{times['Original']:9.2f}ms | ", end="")
                print(f"{times['Enhanced']:9.2f}ms | ", end="")
    ...
    # Find best performer for each sequence length
    best_by_seq_len = {}
    for result in results:
        if result["dilation_rate"] == 1:
            ...
            best_by_seq_len[seq_len] = best

    print("\nBest implementation by sequence length (dense attention):")
    for seq_len, (impl, t) in sorted(best_by_seq_len.items()):
        print(f"  {seq_len:5d} tokens: {impl} ({t:.2f}ms)")

    # Check 8K optimization
    for result in results:
        if result["seq_len"] == 8192 and result["dilation_rate"] == 1:
            times = result["times"]
            if "Enhanced" in times and "Unified Optimized" in times:
                if times["Enhanced"] < times["Unified Optimized"]:
                    improvement = (
                        times["Unified Optimized"] / times["Enhanced"] - 1
                    ) * 100
                    print(
    ...
    print("\nSparse pattern performance (Enhanced vs Original):")
    sparse_speedups = []
    for result in results:
        if result["dilation_rate"] > 1:
            times = result["times"]
            if times["Original"] != float("inf") and times["Enhanced"] != float("inf"):
                ...
                print(f"  Dilation {result['dilation_rate']}: {speedup:.2f}x speedup")

    if sparse_speedups:
        avg_sparse_speedup = sum(sparse_speedups) / len(sparse_speedups)
        print(f"\nAverage sparse speedup: {avg_sparse_speedup:.2f}x")
