...
def benchmark_attention(attention_module, inputs, num_runs=5, warmup=2):
    ...
    for _ in range(warmup):
        with torch.no_grad():
            if hasattr(attention_module, 'forward') and 'need_weights' in attention_module.forward.__code__.co_varnames:
...
def main():
    ...
    results_file = os.path.join("..", "docs", "benchmarks", f"benchmark-comprehensive-results-{timestamp}.md")
    ...
    with open(results_file, "w") as f:
        ...
        f.write("| Sequence Length | DilatedAttention | ImprovedDilatedAttention | MultiheadDilatedAttention | ImprovedMultiheadDilatedAttention |\n")
        f.write("|-----------------|------------------|--------------------------|---------------------------|-----------------------------------|\n")
        
        for seq_len in sequence_lengths:
            row = f"| {seq_len} "
            for impl in ['DilatedAttention', 'ImprovedDilatedAttention', 'MultiheadDilatedAttention', 'ImprovedMultiheadDilatedAttention']:
        ...
        # Calculate speedups
        for seq_len in sequence_lengths:
            if seq_len in results['DilatedAttention'] and seq_len in results['ImprovedDilatedAttention']:
                speedup = results['DilatedAttention'][seq_len] / results['ImprovedDilatedAttention'][seq_len]
                f.write(f"- Sequence {seq_len}: ImprovedDilatedAttention is {speedup:.2f}x faster\n")
