#!/usr/bin/env python3
"""
Proper benchmark for hybrid ring dilated attention WITHOUT all_gather.
Each GPU reports its own metrics independently.

Run with: torchrun --nproc_per_node=2 benchmarks/benchmark_hybrid_proper.py
"""
...
def main():
    ...
    print(f"[GPU {rank}] Starting benchmark...")
    ...
    if results:
        ...
        filename = (
            f"benchmarks/hybrid_results_rank{rank}_{world_size}gpu_{timestamp}.json"
        )
    ...
    if rank == 0 and results:
        print("\n" + "=" * 60)
        print("BENCHMARK SUMMARY (from GPU 0)")
        print("=" * 60)
        ...
        for r in results:
            ...
            print(f"  O(n/p): {r['expected_scaling']:,.0f}")
