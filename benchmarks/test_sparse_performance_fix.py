...
def benchmark_sparse_pattern(impl_class, impl_name, seq_len, dilation_rate, 
                           batch_size=2, hidden_dim=512, num_heads=8,
                           warmup=3, runs=10):
...
def main():
    ...
    # Summary table
    print("\n\n=== Performance Summary ===")
    print(f"{'Config':<20} | {'Unified':<10} | {'Optimized':<10} | {'Enhanced':<10} | {'Status':<15}")
    print("-" * 75)
    ...
    for desc in results:
        ...
        print(f"{desc:<20} | {unified_time:>8.2f}ms | {optimized_time:>8.2f}ms | {enhanced_time:>8.2f}ms | {status}")
    ...
    if avg_improvement > 0.8:  # If Enhanced is at least 80% as fast as Unified
        print("✓ SUCCESS: Enhanced implementation now uses efficient Triton kernel for sparse patterns!")
