#!/usr/bin/env python3
"""
Single-GPU benchmark to reach 128K tokens using optimal extreme dilation configuration.
This is for comparison with multi-GPU results.
"""
...
def benchmark_extreme_dilation_single_gpu(
    seq_len: int,
    batch_size: int = 1,
    num_heads: int = 8,
    head_dim: int = 64,
    ...
    """Benchmark extreme dilation configuration on a single GPU."""
    ...
    print(f"\nTesting {seq_len:,} tokens with extreme dilation (8,16) on SINGLE GPU")
    ...
    try:
        ...
        # Note: ring_size=1 for single GPU (no ring communication)
        model = RingDilatedAttentionHilbertOptimized(
            ...
            ring_size=1,  # Single GPU - no ring
        )
...
def main():
    """Run single-GPU benchmark to reach 128K tokens."""

    print("=" * 80)
    print("SINGLE-GPU BENCHMARK: EXTREME DILATION TO 128K TOKENS (FP32)")
    print("=" * 80)
    ...
    print("- Single GPU (no ring communication)")
    ...
    for seq_len in seq_lengths:
        result = benchmark_extreme_dilation_single_gpu(seq_len)
    ...
    if successful_results:
        ...
        ax1.set_title("Single-GPU Throughput Scaling", fontsize=14)
        ax1.set_xscale("log")
        ax1.set_yscale("log")
        ...
        summary = f"""SINGLE-GPU PERFORMANCE SUMMARY
...
• 2 GPUs: ~{max_achieved * 2:,} tokens possible
• 4 GPUs: ~{max_achieved * 4:,} tokens possible
• 8 GPUs: ~{max_achieved * 8:,} tokens possible
"""
        ...
        plt.suptitle("Single-GPU Extreme Dilation Scaling to 128K Tokens", fontsize=16)
        plt.tight_layout()

        # Save plot
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        filename = f"hilbert_128k_extreme_single_gpu_{timestamp}.png"
    ...
    print("\nConclusion:")
    print("✓ Successfully scaled to 128K tokens on single GPU with FP32")
    ...
    print("✓ Multi-GPU setup recommended for larger sequences")
    print("=" * 80)
