...
def benchmark_implementation(
    ...
    try:
        ...
        print(
            f"  ✓ {impl_name}: {avg_time_ms:.1f}ms, {peak_memory_gb:.3f}GB, {throughput / 1e6:.2f}M tok/s"
        )
...
def test_extreme_sequences():
    ...
    test_sequences = [8192, 16384, 32768, 65536, 131072, 262144, 524288, 1048576]
...
def create_plots(memory_results, extreme_results, perf_results, output_dir):
    ...
    for impl, color, marker in zip(impl_types, colors, markers):
        impl_data = [
            r for r in memory_results if r["implementation"] == impl and r["success"]
        ]
        if impl_data:
            # Group by sequence length, take ring_size=1
            seq_lens = sorted(
                set(r["seq_len"] for r in impl_data if r["ring_size"] == 1)
            )
            memories = [
                next(
                    (
                        r["memory_gb"]
                        for r in impl_data
                        if r["seq_len"] == s and r["ring_size"] == 1
                    ),
                    None,
                )
                for s in seq_lens
    ...
    target_seq = 16384
    for impl, color, marker in zip(
        impl_types[1:], colors[1:], markers[1:]
    ):  # Skip broken impl
        impl_data = [
            r
            for r in memory_results
            if r["implementation"] == impl
            and r["seq_len"] == target_seq
            and r["success"]
        ]
        if impl_data:
            ring_sizes = sorted(set(r["ring_size"] for r in impl_data))
            memories = [
                next((r["memory_gb"] for r in impl_data if r["ring_size"] == rs), None)
...
def main():
        ...
        for impl in ["RingDilatedAttention (current)", "RingDilatedAttentionFixed"]:
            ...
            if impl_data:
                ...
                if ring1 and ring8:
                    ...
                    print(
                        f"    {impl}: {reduction:.1f}% memory reduction with ring_size=8"
                    )
    ...
    for impl in ["RingDilatedAttentionFixed", "TrueRingDilatedAttention"]:
        successful = [
            r for r in extreme_results if r["implementation"] == impl and r["success"]
        ]
        if successful:
            ...
            print(
                f"    Throughput: {max_result['throughput_tokens_per_sec'] / 1e6:.2f}M tok/s"
            )
        ...
        if baseline:
            ...
            for r in perf_8k:
                if r != baseline:
                    ...
                    print(
                        f"    {r['implementation']}: {r['time_ms']:.1f}ms ({overhead:+.1f}% overhead)"
                    )
