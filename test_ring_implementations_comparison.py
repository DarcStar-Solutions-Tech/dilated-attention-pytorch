...
def test_implementation_comparison():
    ...
    if forward_time_single != float('inf') and forward_time_multi != float('inf'):
        ...
        print(f"  Speedup: {speedup:.2f}x ({'multihead faster' if speedup > 1 else 'single-headed faster'})")
...
def test_mathematical_equivalence():
    ...
    with torch.no_grad():
        if hasattr(multihead_attention, 'qkv_proj'):
            ...
            k_proj = qkv[:, :, embed_dim:2*embed_dim].view(batch_size, seq_len, num_heads, head_dim)
            v_proj = qkv[:, :, 2*embed_dim:].view(batch_size, seq_len, num_heads, head_dim)
        else:
            q_proj = multihead_attention.q_proj(query).view(batch_size, seq_len, num_heads, head_dim)
            k_proj = multihead_attention.k_proj(query).view(batch_size, seq_len, num_heads, head_dim)
            v_proj = multihead_attention.v_proj(query).view(batch_size, seq_len, num_heads, head_dim)
...
def test_memory_scaling():
    ...
    for seq_len in sequence_lengths:
        ...
        # Calculate ratio
        ratio = single_memory / multi_memory if multi_memory > 0 and single_memory != float('inf') else 1.0
