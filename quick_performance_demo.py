...
import torch
import time
from dilated_attention_pytorch import DilatedAttention, ImprovedDilatedAttention, MultiheadDilatedAttention
from dilated_attention_pytorch.ring_dilated_attention import RingDilatedAttention
from dilated_attention_pytorch.block_sparse_ring_dilated_attention import BlockSparseRingDilatedAttention, SparsePatternConfig
...
def main():
    ...
    for seq_len, desc in test_configs:
        ...
        segments = [min(1024, seq_len//4), min(2048, seq_len//2), min(4096, seq_len)]
        ...
        try:
            module = RingDilatedAttention(segments, dilation_rates, 0.0, ring_size=1).to(device, dtype)
        ...
        if seq_len <= 32768:
            try:
                ...
                module = BlockSparseRingDilatedAttention(segments, dilation_rates, sparse_config=sparse_config, ring_size=1).to(device, dtype)
        ...
        # Multihead
        try:
            module = MultiheadDilatedAttention(embed_dim, num_heads, dilation_rates, segments, 0.0).to(device, dtype)
