...
def test_deprecated_ring_multihead_attention_warning():
    ...
    with pytest.warns(DeprecationWarning, match="RingMultiheadDilatedAttention is deprecated"):
...
def test_migration_guide_examples():
    ...
    with warnings.catch_warnings(record=True) as w:
        ...
        old_attention = RingDilatedAttention(
    ...
    with warnings.catch_warnings(record=True) as w:
        ...
        new_attention = create_dilated_attention(
            "ring",
            segment_lengths=[2048, 4096],
            dilation_rates=[1, 2],
            ring_size=4,
        ...
        assert len(deprecation_warnings) == 0, "Factory should not emit deprecation warnings"
