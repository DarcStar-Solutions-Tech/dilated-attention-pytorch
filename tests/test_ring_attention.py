...
class RingAttentionTester:
    ...
    def test_memory_complexity(self) -> Dict[str, Dict]:
        ...
        for seq_len in sequence_lengths:
            ...
            try:
                ...
                test_config["segment_lengths"] = [min(s, seq_len//2) for s in test_config["segment_lengths"]]
                
                # Create inputs
                q, k, v = self.create_test_inputs(
                    test_config["batch_size"], test_config["seq_len"],
                    test_config["embed_dim"], test_config["num_heads"]
                )
                ...
                self.log(f"    Time: {metrics['execution_time']:.4f}s, "
    ...
    def test_performance_comparison(self) -> Dict[str, Dict]:
        ...
        for config in self.test_configs[:2]:  # Limit to smaller configs
            ...
            try:
                ...
                if (standard_metrics.get('error') is None and 
                    ring_metrics.get('error') is None):
                    
                    speedup = standard_metrics['execution_time'] / ring_metrics['execution_time']
                    memory_ratio = ring_metrics['gpu_memory_used'] / max(standard_metrics['gpu_memory_used'], 1)
    ...
    def run_all_tests(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Overall success
            all_equiv_tests = list(results['mathematical_equivalence'].values()) + list(results['multihead_equivalence'].values())
