...
def compare_implementations():
    ...
    for config in configs:
        ...
        try:
            ...
            print(f"  Time: {results_orig['avg_time']:.4f}s ± {results_orig['max_time'] - results_orig['min_time']:.4f}s")
        ...
        try:
            ...
            print(f"  Time: {results_improved['avg_time']:.4f}s ± {results_improved['max_time'] - results_improved['min_time']:.4f}s")
        ...
        if results_orig and results_improved:
            ...
            print(f"\nComparison:")
            print(f"  Speedup: {time_speedup:.2f}x {'(Improved faster)' if time_speedup > 1 else '(Original faster)'}")
            print(f"  Memory ratio: {memory_ratio:.2f}x {'(Improved uses more)' if memory_ratio > 1 else '(Improved uses less)'}")
...
def functional_comparison():
    ...
    print("   - ImprovedDilatedAttention: Uses F.scaled_dot_product_attention with automatic backend selection")
    
    print("\n2. Optimization Features:")
    print("   - DilatedAttention: Manual memory management, explicit device handling")
    print("   - ImprovedDilatedAttention: TF32 support, torch.compile integration, automatic backend selection")
    ...
    print("   - ImprovedDilatedAttention: Multiple optimizations (TF32, torch.compile, SDPA backends)")
