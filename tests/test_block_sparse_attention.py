...
# Test configurations
TEST_CONFIGS = {
    "small": {"batch_size": 2, "seq_len": 1024, "num_heads": 4, "head_dim": 32, "embed_dim": 128},
    "medium": {"batch_size": 4, "seq_len": 4096, "num_heads": 8, "head_dim": 64, "embed_dim": 512},
    "large": {
        "batch_size": 2,
        "seq_len": 16384,
        "num_heads": 16,
...
class TestSparsePatternGeneration:
    ...
    def test_pattern_generation(self, pattern_type, sparsity_ratio):
        ...
        config = SparsePatternConfig(
            pattern_type=pattern_type, sparsity_ratio=sparsity_ratio, block_size=128
        )
...
class TestBlockSparseRingDilatedAttention:
    ...
    def test_causal_masking(self):
        ...
        attention = BlockSparseRingDilatedAttention(
            segment_lengths=[512], dilation_rates=[1], sparse_config=sparse_config, device=device
        )
...
class TestBlockSparseRingMultiheadDilatedAttention:
    ...
    def test_adaptive_sparse_creation(self):
        ...
        attention = create_adaptive_sparse_multihead_attention(
            embed_dim=config["embed_dim"], num_heads=config["num_heads"], device=device
        )
...
class TestSparsePatternUtils:
    ...
    @pytest.mark.parametrize(
        "pattern_type", [PatternType.LOCAL_WINDOW, PatternType.DILATED_SPARSE, PatternType.RANDOM]
    )
