...
class AdaptiveMemoryPool:
    ...
    def clear_unused_buffers(self, threshold: int = 100) -> None:
        ...
        with self._access_lock:
            ...
            for key in list(self._access_order):
                ...
                if key in self._pools and self._pools[key]:
                    num_to_evict = min(len(self._pools[key]), buffers_to_evict - evicted)
    ...
    def get_stats(self) -> Dict[str, int]:
        ...
        with self._access_lock:
            ...
            stats["total_buffers"] = sum(len(buffers) for buffers in self._pools.values())
...
class OptimizedGradientCommunicator:
    ...
    def add_gradient(self, grad: Tensor, param_name: Optional[str] = None) -> None:
        ...
        with self._lock:
            ...
            if (self._current_bucket_size + grad_size > self.bucket_size_mb or
                len(self._current_bucket) >= self.bucket_count):
    ...
    def _compress_gradient(self, grad: Tensor, param_name: str) -> Tensor:
        ...
        self._error_feedback[param_name] = (grad_flat - sparse_grad).reshape(original_shape)
        ...
        with self._lock:
            ...
            for handle in self._allreduce_handles:
                ...
                for grad in handle.bucket:
                    numel = grad.numel()
                    grad.copy_(handle.flat_grads[offset:offset + numel].reshape(grad.shape))
                    offset += numel
