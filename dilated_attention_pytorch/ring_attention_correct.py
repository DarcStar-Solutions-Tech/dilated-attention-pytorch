...
class RingAttentionCorrect(nn.Module):
    ...
    def forward(
        ...
        assert n % ring_size == 0, f"Sequence length {n} must be divisible by ring_size {ring_size}"
        ...
        for chunk_idx in range(ring_size):
            ...
            scores = torch.matmul(query.transpose(1, 2), k_chunk.transpose(1, 2).transpose(-2, -1)) / math.sqrt(d)
            
            # Apply causal mask if needed
            if is_causal:
                # Create causal mask accounting for chunk position
                causal_mask = self._create_causal_mask(n, chunk_size, start_idx, query.device)
        ...
        if return_memory_stats and self.device.type == "cuda":
            torch.cuda.synchronize(self.device)
            memory_stats["peak_memory_mb"] = torch.cuda.max_memory_allocated(self.device) / (1024**2)
    ...
    def _create_causal_mask(self, seq_len: int, chunk_size: int, chunk_offset: int, device: torch.device) -> Tensor:
    ...
    def demonstrate_memory_savings(self, seq_len: int = 4096, batch_size: int = 1, 
        ...
        print(f"Sequence length: {seq_len}, Batch: {batch_size}, Heads: {num_heads}, Dim: {head_dim}")
        print("="*70)
        ...
        for ring_size in ring_sizes:
            ...
            output, stats = self.forward(q, k, v, ring_size=ring_size, return_memory_stats=True)
        ...
        for r in results:
            reduction_pct = (1 - r['peak_memory_mb'] / baseline_memory) * 100 if baseline_memory > 0 else 0
            print(f"{r['ring_size']:>10} {r['peak_memory_mb']:>18.1f} {reduction_pct:>19.1f}%")
        
        return results


def compare_with_standard_attention(seq_len: int = 1024, ring_sizes: list = [1, 2, 4, 8]):
    ...
    scores = torch.matmul(q.transpose(1, 2), k.transpose(1, 2).transpose(-2, -1)) / math.sqrt(head_dim)
    ...
    for ring_size in ring_sizes:
        ...
        assert max_diff < 1e-5, f"Ring attention output doesn't match standard attention for ring_size={ring_size}"
