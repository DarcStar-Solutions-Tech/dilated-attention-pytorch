...
import torch
from typing import Sequence, Union, List, Tuple, Optional, Any
...
class ValidationMixin:
    ...
    def validate_segment_dilation_match(segment_lengths: Sequence[int],
                                      dilation_rates: Sequence[int]) -> None:
        """
    ...
    @staticmethod
    def validate_tensor_shapes_match(tensors: List[torch.Tensor],
                                   names: List[str],
                                   dims_to_check: Optional[List[int]] = None) -> None:
        """
        ...
        for i, (tensor, name) in enumerate(zip(tensors[1:], names[1:]), 1):
    ...
    @staticmethod
    def validate_batch_first_format(tensor: torch.Tensor,
                                  name: str,
                                  expected_dims: int = 4) -> None:
        """
    ...
    @staticmethod
    def validate_attention_mask(mask: Optional[torch.Tensor],
                              batch_size: int,
                              seq_len: int,
                              name: str = "attention_mask") -> None:
        """
        ...
        if mask.dim() == 2:
            if mask.shape != (seq_len, seq_len):
                raise ValueError(
                    f"{name} with 2D shape must be ({seq_len}, {seq_len}), "
                    f"got {mask.shape}"
                )
        elif mask.dim() == 4:
            ...
            raise ValueError(
                f"{name} must be either 2D or 4D, got {mask.dim()}D"
            )
    ...
    def validate_dropout_prob(dropout: float, name: str = "dropout") -> None:
        ...
        if not 0.0 <= dropout <= 1.0:
            raise ValueError(
                f"{name} must be between 0.0 and 1.0, got {dropout}"
            )
