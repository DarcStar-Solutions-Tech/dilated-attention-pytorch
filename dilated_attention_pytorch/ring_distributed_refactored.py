...
class RingDistributedDilatedAttention(BaseMultiheadDilatedAttention):
    ...
    def _setup_distributed(self):
        ...
        if not dist.is_initialized():
            warnings.warn(
                "Distributed not initialized. Running in single-GPU mode."
            )
            return
...
def create_ring_distributed_attention(
    ...
    **kwargs
    ...
    multihead_config = MultiheadConfig(
        embed_dim=embed_dim,
        num_heads=num_heads,
        **{k: v for k, v in kwargs.items() if k in ['bias', 'layer_norm', 'gamma_init', 'device', 'dtype']}
    )
    ...
    config = RingDistributedConfig(
        ...
        ring_config=ring_config,
        distributed_config=distributed_config,
        **{k: v for k, v in kwargs.items() if k in ['enable_deepspeed', 'enable_fairscale', 'enable_monitoring']}
    )
