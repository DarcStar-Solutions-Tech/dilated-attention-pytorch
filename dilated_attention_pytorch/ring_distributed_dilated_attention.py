...
class RingDistributedDilatedAttention(nn.Module):
    ...
    def _init_model_parallel_components(
        ...
        self.qkv_proj = ColumnParallelLinear(
            embed_dim, 3 * embed_dim, bias=bias, device=device, dtype=dtype, gather_output=False
        )

        # Output projection with row parallelism
        self.out_proj = RowParallelLinear(
            embed_dim, embed_dim, bias=bias, device=device, dtype=dtype, input_is_parallel=True
        )
            ...
            "compression_training": {
                "weight_quantization": {
                    "shared_parameters": {
                        ...
                        "rounding": "nearest",
                        "fp16_mixed_quantize": {"enabled": False, "quantize_change_ratio": 0.001},
                    }
                },
        ...
        if buffer_key not in self._model_parallel_buffers:
            self._model_parallel_buffers[buffer_key] = {
                ...
                "attn_output": torch.empty(
                    (batch_size, seq_len, self.embed_dim), dtype=query.dtype, device=query.device
                ),
            }
    ...
    def _handle_forward_failure(
        self, error: Exception, query: Tensor, key: Tensor, value: Tensor, is_causal: bool
    ) -> tuple[Tensor, Tensor | None]:
