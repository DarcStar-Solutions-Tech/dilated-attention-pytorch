...
class SimplifiedMemoryPool:
    ...
    def __init__(self, config: Optional[MemoryPoolConfig] = None):
        ...
        # Core storage
        self._free_tensors: Dict[Tuple[torch.Size, torch.dtype], List[Tensor]] = defaultdict(list)
    ...
    def _allocate_from_bucket(
        ...
        size_mb = (torch.tensor(size).prod().item() * torch.finfo(dtype).bits) / (8 * 1024 * 1024)
                ...
                for i, tensor in enumerate(candidates):
                    if tensor.dtype == dtype and tensor.device == device:
                        ...
                        if tensor.numel() >= torch.tensor(size).prod().item():
                            ...
                            return tensor[:torch.tensor(size).prod().item()].reshape(size)
...
def get_global_memory_pool(config: Optional[MemoryPoolConfig] = None) -> SimplifiedMemoryPool:
