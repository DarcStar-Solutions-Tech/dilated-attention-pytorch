...
class UnifiedMemoryPool:
    ...
    def _find_compatible_buffer(self,
                               shape: Tuple[int, ...],
                               dtype: torch.dtype,
                               device: Optional[torch.device],
                               pool: OrderedDict) -> Optional[Tensor]:
        """
    ...
    def _maybe_cleanup(self) -> None:
        ...
        try:
            ...
            if free_ratio < self._aggressive_threshold:
                ...
                logger.debug(f"Moderate memory pressure ({free_ratio:.1%} free), conservative cleanup")
    ...
    def _aggressive_cleanup(self) -> None:
        ...
        with self._lock:
            ...
            logger.info(f"Aggressive cleanup complete, freed memory")
            ...
            for pool_name, pool in self._pools.items():
                ...
                for key in pool:
                    if key in self._stats:
                        ...
                        if (time_since_access > 60 and 
                            key not in self._hot_cache and
                            id(pool[key]) not in self._active_buffers):
                            keys_to_remove.append(key)
