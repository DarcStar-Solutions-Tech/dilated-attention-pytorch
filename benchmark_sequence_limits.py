...
class SequenceLengthBenchmark:
    ...
    def benchmark_implementation(
        ...
        try:
            ...
            if is_multihead:
                ...
                query = torch.randn(batch_size, seq_len, num_heads, head_dim, device=self.device, dtype=self.dtype)
                key = torch.randn(batch_size, seq_len, num_heads, head_dim, device=self.device, dtype=self.dtype)
                value = torch.randn(batch_size, seq_len, num_heads, head_dim, device=self.device, dtype=self.dtype)
    ...
    def find_max_sequence_length(
        ...
        for seq_len in seq_lengths:
            ...
            if result.success:
                ...
                print(f" ✓ {result.time_ms:.1f}ms, {result.memory_gb:.2f}GB, {result.throughput_tokens_per_sec:.0f} tok/s")
            else:
    ...
    def run_comprehensive_benchmark(self):
        ...
        implementations = [
            ...
            ("DilatedAttention", 
             lambda seq_len: DilatedAttention(
                 segment_lengths=[min(1024, seq_len//4), min(2048, seq_len//2), min(4096, seq_len)],
        ...
        for name, factory, is_multihead in implementations:
            ...
            max_len = self.find_max_sequence_length(
                name, factory, seq_lengths, batch_size, num_heads, head_dim, is_multihead
            )
    ...
    def print_use_case_recommendations(self, max_lengths: Dict[str, int]):
        ...
        for name, max_len in sorted(short_seq, key=lambda x: x[1], reverse=True):
            ...
            perf = next((r for r in self.results if r.implementation == name and r.seq_len == 4096 and r.success), None)
            if perf:
                print(f"   - {name}: {perf.time_ms:.1f}ms @ 4K tokens ({perf.throughput_tokens_per_sec:.0f} tok/s)")
        ...
        for name, max_len in sorted(medium_seq, key=lambda x: x[1], reverse=True):
            ...
            perf = next((r for r in self.results if r.implementation == name and r.seq_len == 32768 and r.success), None)
            if perf:
                print(f"   - {name}: {perf.time_ms:.1f}ms @ 32K tokens ({perf.throughput_tokens_per_sec:.0f} tok/s)")
        ...
        for name, max_len in sorted(long_seq, key=lambda x: x[1], reverse=True):
            ...
            largest = max((r for r in self.results if r.implementation == name and r.success), 
                         key=lambda x: x.seq_len, default=None)
            if largest:
                print(f"     Performance: {largest.time_ms:.1f}ms @ {largest.seq_len:,} tokens ({largest.throughput_tokens_per_sec:.0f} tok/s)")
        
        print("\n4. MEMORY EFFICIENCY RANKING (at 32K tokens):")
        mem_results = [(r.implementation, r.memory_gb) 
        ...
        speed_results = [(r.implementation, r.throughput_tokens_per_sec) 
                        for r in self.results 
