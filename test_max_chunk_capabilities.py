...
def test_max_chunk_size_single_headed():
    ...
    for chunk_size in chunk_sizes:
        ...
        try:
            ...
            # Create test tensors
            q = torch.randn(batch_size, chunk_size, num_heads, head_dim, 
                           device=device, dtype=dtype)
            k = torch.randn(batch_size, chunk_size, num_heads, head_dim, 
                           device=device, dtype=dtype)
            v = torch.randn(batch_size, chunk_size, num_heads, head_dim, 
                           device=device, dtype=dtype)
...
def test_max_chunk_size_multihead():
    ...
    for chunk_size in chunk_sizes:
        # Skip if chunk size is unreasonably large
        estimated_memory = (batch_size * chunk_size * embed_dim * 2 * 4) / (1024**3)  # Extra for projections
        ...
        try:
            ...
            # Create test tensors
            query = torch.randn(batch_size, chunk_size, embed_dim, 
                               device=device, dtype=dtype)
...
def test_billion_token_capability():
    ...
    for impl_name, impl_type in implementations:
        ...
        try:
            ...
            print(f"  Estimated total time: {total_time_estimate:.1f}s ({total_time_estimate/3600:.1f} hours)")
            print(f"  Estimated total throughput: {total_throughput:,.0f} tokens/second")
            print(f"  Memory per device: ~{torch.cuda.max_memory_allocated()/(1024**3):.3f}GB" if device.type == "cuda" else "")
