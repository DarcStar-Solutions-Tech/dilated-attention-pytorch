    ...
    for bench_type, results in sorted(results_by_type.items()):
        ...
        if commits:
            ...
            for r in recent_results:
                commit_short = (
                    r["commit"][:8] if r["commit"] != "unknown" else "unknown"
                )
        ...
        for impl, perf_data in impl_performance.items():
            if perf_data:
                ...
                for p in perf_data[-5:]:
                    print(
                        f"    {p['timestamp']}: {p['time_ms']:.2f}ms (commit: {p['commit']})"
                    )
    ...
    if len(results_by_type) == 0:
        ...
        if all_timestamps:
            ...
            try:
                ...
                if days_old > 7:
                    print(
                        f"  - Latest results are {days_old} days old - consider running new benchmarks"
                    )
            except:
                pass
