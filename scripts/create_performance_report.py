                    ...
                    for json_file in timestamp_dir.glob("*.json"):
                        try:
                            ...
                            if "metadata" in data:
                                ...
                                bench_type = data["metadata"].get(
                                    "benchmark_type", type_dir.name
                                )
...
def generate_performance_report():
    ...
    report_lines.extend(["## Commits Analyzed", ""])
    ...
    report_lines.extend(["", "## Performance Comparison: All Implementations", ""])
    ...
    if perf_by_impl:
        report_lines.extend(
            [
                ...
                "",
                "| Implementation | "
                + " | ".join(f"{c[:8]}" for c in commits if c != "unknown")
                + " | Trend |",
                "|----------------|"
        ...
        for impl in sorted(perf_by_impl.keys()):
            ...
            for p in perf_data:
                if (
                    p["commit"] not in commit_perf
                    or p["timestamp"] > commit_perf[p["commit"]]["timestamp"]
                ):
    ...
    report_lines.extend(["", "## Memory Pool Performance", ""])
                    ...
                    mem_pool_data.append(
                        {
                            ...
                            "without_pool": alloc_perf.get(
                                "avg_time_without_pool_ns", 0
                            ),
    ...
    if perf_by_impl:
        ...
        report_lines.extend(["### Fastest Implementations (Latest Commit)", ""])
...
def main():
    ...
    report_path = (
        ...
        / "docs"
        / "reports"
        / f"performance-comparison-{timestamp}.md"
    )
