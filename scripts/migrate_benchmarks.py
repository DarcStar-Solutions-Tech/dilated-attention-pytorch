...
def clean_duplicate_benchmarks():
    ...
    # Files to process
    all_files = (
        list(docs_benchmarks.glob("*.json"))
        + list(docs_benchmarks.glob("*.png"))
        + list(docs_benchmarks.glob("*.md"))
    )
    ...
    for file_path in all_files:
        ...
        match = re.match(
            r"([\w-]+?)-(2\d{3}-\d{2}-\d{2}-\d{4}-UTC)\.(json|png|md)", file_path.name
        )
        if match:
    ...
    for (bench_type, timestamp), files in file_groups.items():
        ...
        for ext, file_path in files.items():
            ...
            if not dest.exists():
                ...
                print(
                    f"  Moved: {file_path.name} → {dest.relative_to(docs_benchmarks)}"
                )
...
def update_latest_links():
    ...
    for type_dir in by_type_dir.iterdir():
        ...
        timestamps = sorted(
            [d.name for d in type_dir.iterdir() if d.is_dir()], reverse=True
        )
        if timestamps:
...
def update_benchmark_script(script_path: Path):
    ...
    replacements = [
        ...
        (r"helper\.save_plot\(([\w\s,]+)\)", r"output_manager.save_plot(\1)"),
        # Add proper imports
        (
            r"(import [\w\s,]+\n)+",
...
def main():
    ...
    print("\n✅ Migration complete!")
    print("   - Organized benchmark files")
    print(f"   - Updated {updated_count} scripts")
    print("   - Created dashboard at docs/benchmarks/index.html")
    print("   - Generated trend report at docs/benchmarks/trend-report.md")
