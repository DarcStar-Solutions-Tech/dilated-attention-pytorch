export const meta = {
  name: 'dilated-attention-pr25-scoped-audit',
  description: 'Scoped correctness audit of the PR #25 branch (feature/critical-improvements-phase1) via 1M-context lens agents',
  phases: [
...
const ROOT = '/home/mharris/Projects/DarcStar-Technologies/dilated-attention-pr25'
...
const LENSES = [
  ...
  },
  {
    ...
    scope: [
      `${PKG}/kernels/`,
      `${PKG}/kernels/hilbert_attention_unified.py`,
      `${PKG}/kernels/hilbert_attention_unified_optimized_enhanced.py`,
      `${PKG}/kernels/hilbert_attention_unified_optimized_enhanced_refactored.py`,
      `${PKG}/kernels/cache_manager.py`,
      `${PKG}/kernels/config_strategies.py`,
    ...
    focus: [
      ...
      'Hilbert index mapping (hilbert_curve.py) vs how the kernels consume it: does the reorder + inverse-reorder round-trip exactly, and is it applied consistently per-segment?',
      'Non-Triton fallback path: does the eager/PyTorch branch compute the SAME result as the Triton kernel, and is the Triton path correctly gated on availability/shape?',
      'DRIFT across the three hilbert_attention_unified* variants (base / optimized_enhanced / optimized_enhanced_refactored): which is canonical and exported via kernels/__init__.py + HilbertAttentionMixin, is a bug fixed in one variant but not the others, is a stale variant still imported or selectable?',
    ].join('\n  - '),
  },
  {
    key: 'sparse-deadcode',
