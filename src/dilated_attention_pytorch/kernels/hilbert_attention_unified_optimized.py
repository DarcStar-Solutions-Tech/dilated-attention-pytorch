#!/usr/bin/env python3
"""
Optimized Unified Hilbert Attention kernel with performance improvements.

This version incorporates optimizations from multiple implementations:
- Segment boundary optimization from hilbert_attention_optimized.py
- Efficient transpose operations from hilbert_attention_core.py
- Better memory access patterns from fused kernels
...
@triton.jit
def unified_hilbert_attention_kernel_optimized(
    # Pointers
    Q,
    K,
    V,
    ...
    """
    Optimized unified attention kernel.

    Key optimizations:
    1. Pre-compute segment boundaries once
    2. Vectorized Hilbert index loading
    3. Efficient transpose using v_t directly
    4. Optional prefetching for better memory bandwidth
    ...
    # All queries in this block are in the same segment
    ...
    if dilation_rate > 1:
        # OPTIMIZATION 2: Sparse attention with efficient position calculation
        # Calculate number of active positions in segment
        num_active = (seg_end - seg_start + dilation_rate - 1) // dilation_rate

        # Process sparse positions
        for block_idx in range(0, num_active, BLOCK_N):
            # Calculate sparse positions directly
            ...
            # OPTIMIZATION 3: Vectorized Hilbert index loading
            ...
            # OPTIMIZATION 5: Use efficient matmul pattern
            s = tl.dot(q, tl.trans(k))
            s = tl.where(mask_n[None, :], s, mask_value)

            # Apply softmax updates inline
...
class UnifiedHilbertAttentionOptimized(nn.Module):
    """
    Optimized Unified Hilbert Attention with performance improvements.
    """
    ...
    def __init__(
        ...
        self._hilbert_cache = BoundedCache(
            ...
            name="UnifiedHilbertAttentionOptimized_cache",
        )
    ...
    def _get_kernel_config(self, seq_len: int) -> Tuple[int, int, int, bool, bool]:
        """
        ...
        """
        # Configurations optimized from benchmarking
        if self.compute_capability < 7:  # Pascal and older
            # For Pascal, we need to ensure BLOCK_D covers the full head_dim
            # to avoid dimension mismatches in matrix multiplication
            block_d = 64 if self.head_dim >= 64 else self.head_dim

            if seq_len <= 1024:
                return 32, 32, block_d, False, False
            elif seq_len <= 2048:
                # Optimize for 2048 - use same BLOCK_M and BLOCK_N for consistency
                return 64, 64, block_d, True, False
            elif seq_len <= 4096:
                return 64, 64, block_d, True, False
            elif seq_len <= 8192:
                return 64, 64, block_d, True, False
            ...
            # Ensure BLOCK_D matches head_dim for proper matrix multiplication
            block_d = self.head_dim

            if seq_len <= 1024:
                return 64, 64, block_d, False, False
            elif seq_len <= 2048:
                # Keep consistent dimensions
                return 128, 128, block_d, True, True
            elif seq_len <= 4096:
                return 128, 128, block_d, True, True
            elif seq_len <= 8192:
                return 128, 128, block_d, True, True
                ...
                return 128, 256, block_d, True, True
    ...
    def forward(
        ...
        """
        Forward pass with optimized unified kernel.
        """
        B, M, D = x.shape
        device = x.device
        ...
        if M_padded <= 512 or is_causal or not self._triton_available:
            ...
            # Use optimized unified Triton kernel
            BLOCK_M, BLOCK_N, BLOCK_D, USE_FUSED_SOFTMAX, ENABLE_PREFETCH = (
                self._get_kernel_config(M_padded)
            )
            ...
            out = self._triton_forward_optimized(
                q,
                k,
                v,
                hilbert_map,
    ...
    def _triton_forward_optimized(
        ...
        """Launch the optimized unified Triton kernel."""
        B, H, M, D = q.shape
        ...
        # Launch optimized kernel
        unified_hilbert_attention_kernel_optimized[grid](
            q,
            k,
            v,
            out,
