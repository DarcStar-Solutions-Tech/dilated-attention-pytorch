...
def benchmark_all_implementations():
    ...
    for batch_size, seq_len, num_heads, head_dim, segments, dilations, size_name in configs:
        ...
        opt_unfold_module = OptimizedUnfoldRingDilatedAttention(segments, dilations, 0.0, ring_size=1).to(device, dtype)
    ...
    print("SUMMARY:")
    print("-" * 80)
    print(f"{'Size':<10} {'Seq Len':<10} {'Original':<12} {'Optimized':<12} {'Speedup':<10} {'Correct':<10}")
    print("-" * 80)
    for r in results:
        print(f"{r['size']:<10} {r['seq_len']:<10} {r['orig_time']:<12.2f} {r['opt_time']:<12.2f} {r['speedup']:<10.2f} {'✓' if r['correct'] else '✗':<10}")
